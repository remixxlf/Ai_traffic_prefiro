...
const StatusBadge = ({ status }: { status: string }) => {
  const config: Record<string, { color: string; icon: string; label: string }> = {
    CONECTADO: { color: 'bg-emerald-50 text-emerald-700 border-emerald-200', icon: '🟢', label: 'Conectado' },
    PENDENTE: { color: 'bg-amber-50 text-amber-700 border-amber-200', icon: '🟡', label: 'Pendente' },
    DESCONECTADO: { color: 'bg-rose-50 text-rose-700 border-rose-200', icon: '🔴', label: 'Desconectado' },
  };
    ...
    <div className="bg-white rounded-2xl border border-slate-200/80 overflow-hidden shadow-sm">
      ...
      <div className="p-6 border-b border-slate-100">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-slate-900 font-semibold">Meta Ads</h3>
              <p className="text-slate-500 text-xs">Facebook & Instagram Ads</p>
            </div>
          </div>
      ...
      {status?.meta.connected && (
        <div className="px-6 py-2">
          <p className="text-slate-500 text-xs font-semibold uppercase tracking-wider mb-2 mt-3">Ativos Vinculados</p>
      ...
      <div className="p-6 border-t border-slate-100">
        {status?.meta.connected ? (
          <div className="flex gap-3">
            <button
              onClick={fetchStatus}
              className="flex-1 px-4 py-2.5 rounded-xl bg-slate-100 text-slate-700 text-sm font-medium hover:bg-slate-200 transition-all border border-slate-200"
            >
              🔄 Sincronizar Ativos
            </button>
            <button
              ...
              className="px-4 py-2.5 rounded-xl border border-rose-200 text-rose-600 text-sm font-medium hover:bg-rose-50 transition-all disabled:opacity-50"
            >
          ...
          <button
            ...
            className="w-full px-4 py-3 rounded-xl bg-indigo-600 text-white font-medium hover:bg-indigo-700 transition-all flex items-center justify-center gap-2 shadow-sm"
          >
