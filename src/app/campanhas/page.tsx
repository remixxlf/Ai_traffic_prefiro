  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight mb-1">Campanhas de Vendas</h1>
          <p className="text-slate-500 text-sm">
            Gerencie e acompanhe o desempenho dos seus anúncios no Meta Ads.
          </p>
        </div>
        <div className="flex items-center gap-2 bg-slate-50 px-3 py-2 rounded-xl border border-slate-200">
          <span className="text-xs text-slate-500 font-medium">ID da Empresa:</span>
          <input
            type="text"
            value={empresaId}
            onChange={(e) => setEmpresaId(e.target.value)}
            placeholder="Cole o ID da empresa"
            className="bg-white text-slate-900 text-xs px-2.5 py-1.5 rounded-lg border border-slate-200 focus:outline-none focus:border-indigo-600 focus:ring-2 focus:ring-indigo-500/10 w-48 font-mono shadow-sm"
          />
        </div>
      </div>
      ...
      {empresaId ? (
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 p-12 text-center text-slate-500 shadow-sm max-w-xl mx-auto">
          ...
          <p className="text-xs text-slate-500 mb-4">
            Informe o ID da sua empresa acima ou selecione uma empresa na página inicial.
          </p>
          <Link
            href="/"
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="font-semibold text-sm text-slate-900">Campanhas</span>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="p-4 sm:p-6 lg:p-8">
        <Suspense fallback={<div className="text-slate-500 text-center p-12 font-medium">Carregando campanhas...</div>}>
          <CampanhasContent />
        </Suspense>
      </main>
    </div>
