...
import PolicyEngineSettings from '@/components/automations/PolicyEngineSettings';
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white mb-1">Níveis de Automação & Guardrails</h1>
          <p className="text-gray-400 text-sm">
            Configure o nível de autonomia da Inteligência Artificial e os limites de segurança orçamentária.
          </p>
        </div>
      ...
      {empresaId ? (
        <PolicyEngineSettings empresaId={empresaId} />
      ) : (
        <div className="bg-gray-900 rounded-2xl border border-gray-800 p-8 text-center text-gray-400">
          ...
          <p className="text-xs text-gray-500">
            Informe o ID da sua empresa no campo acima ou acesse através de <code className="text-blue-400">/automacoes?empresaId=SEU_ID</code>.
          </p>
        </div>
      )}
    </div>
...
export default function AutomacoesPage() {
  return (
    <div className="min-h-screen bg-gray-950 p-6">
      <Suspense fallback={<div className="text-gray-400 text-center p-8">Carregando configurações de automação...</div>}>
        <AutomacoesContent />
      </Suspense>
    </div>
  );
