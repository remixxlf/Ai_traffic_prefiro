...
import CopywritingGenerator from '@/components/copywriting/CopywritingGenerator';
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white mb-1">Textos de Alta Conversão (Copywriting IA)</h1>
          <p className="text-gray-400 text-sm">
            Gere anúncios persuasivos personalizados para a realidade do seu delivery, usando a inteligência do cardápio e memória do negócio.
          </p>
        </div>
      ...
      {empresaId ? (
        <CopywritingGenerator empresaId={empresaId} />
      ) : (
        <div className="bg-gray-900 rounded-2xl border border-gray-800 p-8 text-center text-gray-400">
          ...
          <p className="text-xs text-gray-500">
            Informe o ID da sua empresa no campo acima ou acesse através de <code className="text-blue-400">/copywriting?empresaId=SEU_ID</code> para carregar a memória do negócio e cardápio.
          </p>
        </div>
      )}
    </div>
...
export default function CopywritingPage() {
  return (
    <div className="min-h-screen bg-gray-950 p-6">
      <Suspense fallback={<div className="text-gray-400 text-center p-8">Carregando gerador de textos...</div>}>
        <CopywritingContent />
      </Suspense>
    </div>
  );
